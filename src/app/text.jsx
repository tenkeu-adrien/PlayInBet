...
export default function DashboardPage() {
  ...
  const toggleLeftSidebar = () => {
    ...
    // toast.info(`Menu ${leftSidebarExpanded ? 'fermé' : 'ouvert'}`, {
  ...
  const toggleRightSidebar = () => {
    ...
    // toast.info(`Chat ${rightSidebarExpanded ? 'fermé' : 'ouvert'}`, {
    //   position: isMobile ? 'top-center' : 'top-right'
    // });
  };
  ...
  const handleWithdraw = (e) => {
    ...
    if (amount && amount > 0 && amount <= walletBalance) {
      ...
      setWithdrawAmount('');
      toast.success(`Retrait de $${amount.toFixed(2)} effectué!`, {
        position: isMobile ? 'top-center' : 'top-right'
      });
    } else {
  ...
  const handleBuyItem = (item) => {
    if (walletBalance >= item.price) {
      ...
      toast.success(`Achat réussi: ${item.name}!`, {
        position: isMobile ? 'top-center' : 'top-right'
      });
    } else {
  ...
  const changeTab = (tab) => {
    ...
    // toast.info(`Onglet ${getTabName(tab)} ouvert`, {
    //   position: isMobile ? 'top-center' : 'top-right'
    // });
  };
  ...
  const getTabName = (tab) => {
    switch (tab) {
      ...
      case 'tournaments': return 'Tournois';
      case 'wallet': return 'Portefeuille';
      case 'withdraw': return 'Retrait';
      case 'create-duel': return 'Création de Duel';
  ...
  const DuelCard = ({ duel }) => {
    ...
    const handleJoinDuel = () => {
      if (duel.status === 'waiting') {
        toast.success('Duel rejoint avec succès!', {
          position: isMobile ? 'top-center' : 'top-right'
        });
      } else {
        toast.info('Détails du duel affichés', {
          position: isMobile ? 'top-center' : 'top-right'
        });
      }
    };
  ...
  const TournamentCard = ({ tournament }) => {
    const handleJoinTournament = () => {
      if (tournament.status === 'registration') {
        toast.success('Inscription au tournoi confirmée!', {
          position: isMobile ? 'top-center' : 'top-right'
        });
      } else {
        toast.info('Détails du tournoi affichés', {
          position: isMobile ? 'top-center' : 'top-right'
        });
      }
    };
  ...
  const ProfileTab = () => {
    const handleEditProfile = () => {
      toast.info('Ouverture de l\'éditeur de profil', {
        position: isMobile ? 'top-center' : 'top-right'
      });
    };
  ...
  const LeaderboardTab = () => {
    ...
    const handlePageChange = (direction) => {
      toast.info(`Page ${direction === 'next' ? 'suivante' : 'précédente'}`, {
        position: isMobile ? 'top-center' : 'top-right'
      });
    };
        ...
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center">
              ...
              <select 
                className="bg-white/5 text-white border border-white/10 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                onChange={(e) => toast.info(`Tri appliqué: ${e.target.value}`)}
              >
                <option>Gains</option>
            ...
            <table className="w-full">
              ...
              <tbody>
                {leaderboardData.map(player => (
                  <tr 
                    key={player.rank} 
                    className={`border-b border-white/10 ${player.rank <= 3 ? 'bg-gradient-to-r from-indigo-500/10 to-purple-500/10' : ''}`}
                    onClick={() => toast.info(`Profil de ${player.username} ouvert`)}
                  >
                    <td className="py-4">
          ...
          {!isMobile && (
            <div className="relative hidden md:block">
              ...
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && toast.info('Recherche effectuée')}
              />
            </div>
          )}
        </div>
        ...
        <div className="flex items-center space-x-3 md:space-x-6">
          <div className="flex items-center space-x-2 md:space-x-4">
            <Link
              ...
              title="Connexion"
              onClick={() => toast.info('Ouverture de la page de connexion')}
            >
            ...
            <Link
              ...
              className="flex items-center p-2 rounded-lg text-white hover:bg-white/10 transition-colors"
              title="Inscription"
              onClick={() => toast.info('Ouverture de la page d\'inscription')}
            >
          ...
          <div className={`absolute inset-0 ${slides[currentSlide].bgImage} bg-cover bg-center transition-opacity duration-500`}>
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center flex-col text-center px-4 md:px-8">
              ...
              <button 
                ...
                onClick={() => toast.info('Action du slideshow déclenchée')}
              >
            ...
            {chats.map((chat) => (
              <div 
                ...
                onClick={() => {
                  ...
                  if (isMobile) {
                    toast.info(`Conversation avec ${chat.user} ouverte`);
                  }
                }}
              >
